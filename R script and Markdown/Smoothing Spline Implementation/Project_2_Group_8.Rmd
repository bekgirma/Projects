...
author: "Group #8 - Led Zeppelin"
date: "Joaquin Baquerizo, Bekalu Debelu, Leo DiPerna, Minji Kim, Daniel Sabanov"
output:
  beamer_presentation:
    theme: "Rochester"
    colortheme: "beaver"
...
- In fact, g would just be the linear least squares line, since the loss function ( _penalty function_ ) amounts to minimizing the residual sum of squares.
...
the faster of the two methods with Loess being about three times slower.
...
The \textbf{knots} are the boundaries for each piecewise polynomial function
that the spline is composed of. At each knot (other than the first and last),
the polynomial function changes.
...
The data was collected using web scraping sourced from Sports Reference. The 
...
Link: https://drive.google.com/file/d/1Lx8WHBQITk9UgTlnnaD_WQ40KH-l-MNT
...
ggplot(data = players, mapping = aes(x = MP_PER_G, y = PF_PER_G)) +
  geom_point() +
  geom_line(data=ss.df, aes(x=x, y=mu), color="red") + labs(x="", y="") + 
  ggtitle("Fouls vs. Minutes Played Spline By Hand") +
  ylab("Personal Fouls Per Game") +
  xlab("Minutes Played Per Game") +
  theme_bw()
```
...
library(stats)
ss.fit.1 <- 
  smooth.spline(players$MP_PER_G, players$PF_PER_G, cv = F)
```
  ...
  geom_line(mapping = 
    ...
    geom_line(mapping = 
              aes(x = MP_PER_G, 
                  ...
                  color = "0"), show.legend = F, linewidth=1, alpha = 0.75) +
  
    geom_line(mapping = 
              aes(x = MP_PER_G, 
                  y = predict(ss.fit.3, players$MP_PER_G)$y,
                  color = "10000"), linewidth=1, alpha = 0.75) + 
    ggtitle("Fouls vs. Minutes Played - Smoothing Splines") +
    ylab("Personal Fouls Per Game") +
    xlab("Minutes Played Per Game") +
    theme_bw()
